...
public class Solution
{
    public static void main(String[] args) throws IOException
    {
        ...
        ArrayList<String> listNew2and1 = new ArrayList<String>();
        for (int i = 0; i < list.size(); i++)
        {
            String s = list.get(i);
            if ((s.length()%2)==1)
            listNew2and1.add(s + " " + s + " " + s);
            if ((s.length()%2)==0)
                listNew2and1.add(s + " " + s);
        }

        for (int i = 0; i < listNew2and1.size(); i++)
        {
            System.out.println(listNew2and1.get(i));
        }
